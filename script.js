alert("TU É BIXO DA USP, ENTÃO COLOCA SEUS SEMELHANTES AI! \n                              ou seja coloca um animal")
alert("não necessariemnte precisa preencher todas as letras")
// Lista de listas: cada item representa uma tentativa
  ...
  // Palavra secreta: humano
  const palavraSecreta = ['H', 'U', 'M', 'A', 'N', 'O'];
  ...
  function verificarStatus() {
      ...
      // Coleta apenas as letras preenchidas
      const letrasPreenchidas = [];
      inputs.forEach(input => {
          if (input.value.trim() !== '') {
              letrasPreenchidas.push(input.value.toUpperCase());
          }
      });

      // Verifica se a palavra digitada coincide com a palavra secreta
      const palavraDigitada = letrasPreenchidas.join('');
      const palavraSecretaString = palavraSecreta.join('');
      let status_do_jogo = (palavraDigitada === palavraSecretaString);

      // Verifica cada letra preenchida
      inputs.forEach((input, index) => {
          const letraDigitada = input.value.toUpperCase();
          
          if (letraDigitada === '') {
              // Se a caixa está vazia, não adiciona status
              status.push('');
              return;
          }

          // Verifica se existe uma letra correspondente na palavra secreta
          if (index < palavraSecreta.length && letraDigitada === palavraSecreta[index]) {
      ...
      if (e.key === 'Enter') {
          ...
          inputs[7].value !== ''
              ...
              if(status_do_jogo){
                  ...
                  alert('Parabéns, o bixo acertou o bicho')
                  return; // PARA O JOGO
              }
